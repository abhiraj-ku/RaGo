...
// Wrapper over the openAI sdk, Why? because we can use or swap with any other provider
// otherwise we would have to hardwire everything to one provider only.
type Client struct {
	cfg config.Config
	sdk openai.Client
}
...
// ChatStream: we use this to stream the response
// because we cant let user sit idle so we stream data as it arrives in chunk and string.builder
// efficiently accumulates chunks into a final Message returned to the caller.
...
// this will send the message entire history as input because the model need the
// context of our prev. we translate them to vendor specific objects
